...
function loadCustomerAddressesFromAuth(addresses) {
            ...
            <div class="card-body p-2">
                <div class="form-check">
                    ...
                    <label class="form-check-label w-100" for="address_${address.id}">
                        ...
                        ${address.is_primary ? '<span class="badge ms-1" style="background-color: #ff6b35; color: white;">Principal</span>' : ''}
                        <br>
    ...
    // Setup event listeners for address selection after DOM is updated
    setTimeout(() => {
...
function createPagomovilForm(bankName, accountNumber, pagomovil, holderName, notes, configurations = []) {
    ...
    return `
        <div class="pagomovil-form">
            ...
            <!-- Validate Button -->
            <div class="d-grid">
                <button type="button" class="btn btn-success" onclick="validatePagomovilPayment()" style="background-color: #ff6b35; border-color: #ff6b35; color: white;">
...
function createPagomovilFormInAccordion(bankName, accountNumber, pagomovil, holderName, notes, configurations = []) {
    ...
    return `
        <div class="pagomovil-form-accordion">
            ...
            <!-- Validate Button -->
            <div class="d-grid">
                <button type="button" class="btn btn-sm btn-success" onclick="validatePagomovilPaymentAccordion()" style="background-color: #ff6b35; border-color: #ff6b35; color: white;">
                    <i class="fas fa-check-circle me-2"></i>Registrar Pago
                </button>
            </div>
        </div>
    ...
    return `
        <div class="payment-form-card efectivo-form">
            ...
            <div id="changeSection" class="mb-3" style="display: none;">
                <div class="change-calculation-card bg-white border rounded p-3 shadow-sm">
                    <div class="change-header mb-3">
                        <h6 class="mb-0" style="color: #ff6b35;">
                            <i class="fas fa-calculator me-2"></i>Vuelto Calculado
                        </h6>
                    </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="change-item">
                                    <label class="form-label text-muted small">Monto a pagar:</label>
                                    <div class="change-amount text-dark fw-bold">
                                        $<span id="amountToPay">${remainingAmount.toFixed(2)}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="change-item">
                                    <label class="form-label text-muted small">Vuelto a entregar:</label>
                                    <div class="change-amount text-success fw-bold">
                    ...
                    <div id="pagomovilChangeSection" class="mt-3" style="display: none;">
                        <div class="alert border rounded p-3 mb-3" style="background-color: #fff3e0; border-color: #ff6b35 !important;">
                            <div class="d-flex align-items-center">
                                <i class="fas fa-mobile-alt me-2" style="color: #ff6b35;"></i>
                                <span class="fw-medium" style="color: #ff6b35;">
...
=======
            ...
            <!-- Add Payment Button -->
            <div class="d-grid">
                <button type="button" class="btn btn-sm" onclick="addEfectivoPayment()" style="background-color: #ff6b35; border-color: #ff6b35; color: white;">
...
function createEfectivoBolivaresForm() {
    ...
    if (isCustomerAuthenticated && customerFrequentPaymentMethods.length > 0) {
        frequentMethodsHTML = `
            <div class="frequent-methods-section mb-3">
                ...
                <div class="frequent-methods-grid">
                    ${customerFrequentPaymentMethods.map((method, index) => `
                        <div class="frequent-method-card" onclick="selectFrequentMethod(${index}, 'bolivares')">
                            <div class="method-info">
                                <div class="method-phone">${method.phone_number}</div>
                                <div class="method-details">
                                    <small>${method.cedula} - ${getBankNameByCode(method.bank_code)}</small>
    ...
    return `
        <div class="payment-form-card efectivo-bolivares-form bg-white border rounded p-3 shadow-sm">
            ...
            <div class="currency-conversion mb-3">
                <div class="conversion-info">
                    <div class="row g-3">
                        <div class="col-md-4">
                            <div class="conversion-item">
                                <label class="form-label text-muted small">Total del Pedido (USD)</label>
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="number" class="form-control form-control-sm" value="${orderTotal.toFixed(2)}" readonly>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="conversion-item">
                                <label class="form-label text-muted small">Monto Pendiente (USD)</label>
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="number" class="form-control form-control-sm" value="${remainingAmount.toFixed(2)}" readonly>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="conversion-item">
                                <label class="form-label text-muted small">Tasa de Cambio</label>
                                <div class="input-group">
                                    <input type="number" class="form-control form-control-sm" value="${exchangeRate}" readonly>
                                    <span class="input-group-text">Bs/$</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row mt-2">
                        <div class="col-md-6">
                            <div class="conversion-item">
                                <label class="form-label text-muted small">Monto Pendiente (Bolívares)</label>
                                <div class="input-group">
                                    <span class="input-group-text">Bs.</span>
                                    <input type="number" class="form-control form-control-sm" id="bolivaresPendiente" value="${remainingAmountInVes}" readonly>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div id="bolivaresChangeSection" class="mb-3" style="display: none;">
                <div class="bg-white border rounded p-3 shadow-sm">
                    <h6 class="mb-0" style="color: #ff6b35;"><i class="fas fa-exchange-alt me-2"></i>Vuelto Calculado</h6>
                    <div class="row g-3 mt-2">
                        <div class="col-md-6">
                            <div class="change-item">
                                <label class="form-label text-muted small">Monto a pagar (Bs.):</label>
                                <div class="change-amount text-dark fw-bold">
                                    Bs. <span id="bolivaresAmountToPay">${remainingAmountInVes}</span>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="change-item">
                                <label class="form-label text-muted small">Vuelto a entregar (Bs.):</label>
                                <div class="change-amount text-success fw-bold">
                                    Bs. <span id="bolivaresChangeAmount">0.00</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div id="pagomovilBolivaresChangeSection" class="mt-3" style="display: none;">
                        <div class="alert border rounded p-3 mb-3" style="background-color: #fff3e0; border-color: #ff6b35 !important;">
                            <div class="d-flex align-items-center">
                                <i class="fas fa-mobile-alt me-2" style="color: #ff6b35;"></i>
                                <span class="fw-medium" style="color: #ff6b35;">
            ...
            <!-- Add Payment Button -->
            <div class="d-grid">
                <button type="button" class="btn btn-sm" onclick="addBolivaresPayment()" style="background-color: #ff6b35; border-color: #ff6b35; color: white;">
                    <i class="fas fa-plus-circle me-2"></i>Registrar Pago
                </button>
            </div>
        </div>
...
async function validatePagomovilPayment() {
    ...
    if (!reference) {
        ...
        document.getElementById('pagomovilReference').focus();
        return;
    }
    
    if (reference.length !== 6 || !/^\d{6}$/.test(reference)) {
        showToast('La referencia debe tener exactamente 6 dígitos numéricos', 'error');
        document.getElementById('pagomovilReference').focus();
        document.getElementById('pagomovilReference').select();
        return;
    }
    
    if (!amount || amount <= 0) {
        showToast('Por favor ingrese un monto válido', 'error');
        document.getElementById('pagomovilAmount').focus();
        return;
    }
    
    if (amount < 0.01) {
        showToast('El monto mínimo es de Bs. 0.01', 'error');
        document.getElementById('pagomovilAmount').focus();
        return;
    }
...
// Hide new address form when existing address is selected
function hideNewAddressFormOnSelection() {
...
function setupAddressSelectionListeners() {
    // Remove existing event listeners to prevent duplicates
    const addressRadios = document.querySelectorAll('input[name="selected_address"]');
    addressRadios.forEach(radio => {
        // Remove existing listeners
        radio.removeEventListener('change', hideNewAddressFormOnSelection);
        // Add new listener
        radio.addEventListener('change', function() {
            if (this.checked) {
                hideNewAddressFormOnSelection();
            }
        });
    });
}
