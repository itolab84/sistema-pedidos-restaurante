...
function addEfectivoPayment() {
    const amountToAdd = parseFloat(document.getElementById('efectivoAmount').value || 0);
    const amountReceived = parseFloat(document.getElementById('efectivoReceivedAmount').value || 0);
    
    if (!amountToAdd || amountToAdd <= 0) {
        showToast('Por favor ingrese un monto válido a agregar', 'error');
        return;
    }
    ...
    if (amountToAdd > remainingAmount) {
        showToast(`El monto no puede ser mayor al pendiente ($${remainingAmount.toFixed(2)})`, 'error');
        return;
    }
    
    // Calculate change if applicable
    let changeInfo = null;
    if (amountReceived > 0 && amountReceived > amountToAdd) {
        const change = amountReceived - amountToAdd;
        changeInfo = {
            amount_received: amountReceived,
            change: change,
    ...
    document.getElementById('efectivoReceivedAmount').value = '';
    document.getElementById('changeSection').style.display = 'none';
    
    showToast(`Pago en efectivo de $${amountToAdd.toFixed(2)} agregado exitosamente`, 'success');
}
...
function loadBanksIntoSelect() {
    ...
    // If we don't have banks loaded, load them
